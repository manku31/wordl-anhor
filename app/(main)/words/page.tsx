...
import MOCK_WORDS from "@/lib/mock-words";
...
export default function MyWordsPage() {
  ...
  // Local fav / done state (will be persisted to DB later)
  const [favs, setFavs] = useState<Set<number>>(
    () => new Set(MOCK_WORDS.filter((w) => w.fav).map((w) => w.id)),
  );
  const [dones, setDones] = useState<Set<number>>(
    () => new Set(MOCK_WORDS.filter((w) => w.done).map((w) => w.id)),
  );
  ...
  let words = MOCK_WORDS.filter((w) =>
    w.word.toLowerCase().includes(query.toLowerCase()),
  );
  if (filter === "latest") words = [...words].sort((a, b) => b.id - a.id);
  if (filter === "oldest") words = [...words].sort((a, b) => a.id - b.id);
  if (filter === "favorites") words = words.filter((w) => favs.has(w.id));
  if (filter === "mastered") words = words.filter((w) => dones.has(w.id));

  const selectedWord = MOCK_WORDS.find((w) => w.id === selectedId);
  const selectedIndex = MOCK_WORDS.findIndex((w) => w.id === selectedId);
        ...
        {words.length === 0 ? (
          ...
          >
            {words.map((w, i) => {
              ...
              return (
                // Outer wrapper handles stagger animation + badge/button positioning
                // so they live OUTSIDE the overflow-hidden card and never clip content
                <motion.div key={w.id} variants={item} className="relative">
                  ...
                  {/* Action buttons — float outside card at bottom-right */}
          ...
          <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-6 pointer-events-none">
              <div className="relative w-full max-w-sm pointer-events-auto flex gap-3">
                ...
                {/* Action column — fades out quickly on close */}
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
